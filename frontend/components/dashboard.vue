    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 -mt-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-indigo-500">
          ...
          <p class="text-2xl font-bold mt-2">{{ formatNumberShort(wallet.tokenBalance) }} HIVE</p>
        </div>
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-purple-500">
          <h3 class="text-gray-500 text-sm font-medium">Staked Amount</h3>
          <p class="text-2xl font-bold mt-2">{{ formatNumberShort(wallet.stakedAmount) }} HIVE</p>
        </div>
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-green-500">
          <h3 class="text-gray-500 text-sm font-medium">Pending Rewards</h3>
          <p class="text-2xl font-bold mt-2">{{ formatNumberShort(wallet.pendingRewards) }} HIVE</p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-10">
        <div class="border-b border-gray-200">
          <nav class="flex -mb-px">
            <button 
              ...
              :class="[activeTab === tab.id ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'whitespace-nowrap py-4 px-6 border-b-2 font-medium text-sm']"
            >
              {{ tab.name }}
            </button>
          </nav>
        ...
        <div class="p-6">
          <div v-show="activeTab === 'stake'">
            <amountToStake :formatedtokenBalance="formatNumberShort(wallet.tokenBalance)" :wallet=wallet />
          </div>
          <div v-show="activeTab === 'transfer'">
            <transferHive :formatedtokenBalance="formatNumberShort(wallet.tokenBalance)"  :wallet=wallet />
          </div>
          <div v-show="activeTab === 'withdraw'">
            <withdrawHive :formatedStakedAmount="formatNumberShort(wallet.stakedAmount)" :wallet=wallet />
          </div>
          <div v-show="activeTab === 'claim'">
            <claimReward :formatedPendingRewards="formatNumberShort(wallet.pendingRewards)" :wallet=wallet />
          </div>
        </div>
      </div>
